  ...
  return (
    <span
      className={[
        ...
        on ? 'border-soleil-500' : 'border-border',
      ].join(' ')}
    >
      {on && <span className="bg-soleil-500 h-[9px] w-[9px] rounded-full" />}
    </span>
  );
}
...
export function VisibilitySelector({ value, onChange }: VisibilitySelectorProps) {
  ...
  return (
    <div className="border-border bg-terre-25 overflow-hidden rounded-[14px] border-[0.5px]">
      {/* Option org-wide */}
      <button
        type="button"
        onClick={() => onChange(null)}
        className={`flex w-full items-center gap-2.5 p-3 text-left transition-colors ${orgWide ? 'bg-soleil-50' : 'bg-transparent'}`}
      >
        <span
          className={`inline-flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[9px] ${orgWide ? 'bg-soleil-100 text-soleil-700' : 'bg-terre-100 text-terre-600'}`}
        >
        ...
        <div className="flex-1">
          ...
          <div className="text-terre-500 mt-px text-[11px]">
      ...
      {/* Option restreinte */}
      <div
        className={`flex items-center gap-2.5 p-3 ${!orgWide ? 'bg-soleil-50' : 'bg-transparent'}`}
      >
        <span
          className={`inline-flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-[9px] ${!orgWide ? 'bg-soleil-100 text-soleil-700' : 'bg-terre-100 text-terre-600'}`}
        >
        ...
        <div className="flex-1">
          <div className={`text-terre-900 text-[13px] font-medium ${!orgWide ? 'mb-2' : ''}`}>
        ...
        {orgWide ? (
          <button
            ...
            className="text-soleil-700 shrink-0 text-[12px] font-medium disabled:opacity-40"
          >
            Choisir
          </button>
        ) : (
