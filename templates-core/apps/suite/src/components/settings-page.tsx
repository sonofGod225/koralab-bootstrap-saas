 ...
 *    responsive) avec un fragment italique brand + subtitle + slot actions.
 ...
 *  - [[SaveSuccessBanner]] : bandeau success "Modifications enregistrées"
 */
...
export interface SettingsPageHeaderProps {
  ...
  breadcrumbs?: string[];
  /** Surtitre 11px uppercase tracking-[1.4px] base-500. */
  eyebrow?: string;
  /** Titre principal — texte normal. */
  title: ReactNode;
  /** Fragment italique brand ajouté au titre (ex: "qui vous représente."). */
  italic?: string;
  /** Sous-titre 14px base-600, max-w 640. */
  subtitle?: ReactNode;
    ...
    <header className="mb-5 sm:mb-7">
      ...
      <div className="mb-3.5 flex items-center gap-2">
        {renderNav ? (
          <Sheet open={navOpen} onOpenChange={setNavOpen}>
            <button
              ...
              className="border-border text-base-700 hover:bg-base-100 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] border transition-colors lg:hidden"
            >
        ...
        {breadcrumbs && breadcrumbs.length > 0 ? (
          <nav
            ...
            className="text-base-500 flex flex-1 items-center gap-1.5 overflow-x-auto text-xs whitespace-nowrap"
          >
            {breadcrumbs.map((b, i) => {
              ...
              return (
                <span key={`${b}-${i}`} className="inline-flex items-center gap-1.5">
                  {i > 0 ? <ChevronRight className="text-base-400 h-3 w-3" /> : null}
                  {clickable ? (
                    <button
                      type="button"
                      onClick={() => void navigate({ to: href })}
                      className="text-base-500 hover:text-base-800 cursor-pointer font-normal underline-offset-2 transition-colors hover:underline"
                    >
                      {b}
                    </button>
                  ) : (
                    <span
                      className={
                        isLast ? 'text-base-800 font-medium' : 'text-base-500 font-normal'
                      }
                    >
                      {b}
                    </span>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between sm:gap-5">
        <div className="min-w-0">
          {eyebrow ? (
            <div className="text-base-500 mb-2.5 text-[11px] font-semibold tracking-[1.4px] uppercase">
              {eyebrow}
            </div>
          ) : null}
          <h1 className="font-display text-base-900 m-0 max-w-[640px] text-[28px] leading-[1.1] font-light tracking-[-1px] sm:text-[34px] sm:leading-[1.08] sm:tracking-[-1.2px] lg:text-[40px] lg:leading-[1.05] lg:tracking-[-1.5px]">
            {title}
            {italic ? (
              <>
                {' '}
                <em className="font-display text-brand-600 font-normal italic">{italic}</em>
              </>
            ) : null}
          </h1>
          {subtitle ? (
            <p className="text-base-600 mt-3 max-w-[640px] text-sm">{subtitle}</p>
          ) : null}
        </div>
        {actions ? (
...
export interface SectionCardProps {
  ...
  title?: string;
  /** Description sous le titre (14px base-600, max-w 540). */
  description?: string;
    ...
    >
      {hasHeader ? (
        ...
        >
          <div className="min-w-0">
            {title ? (
              <h3 className="font-display text-base-900 m-0 text-[16px] font-medium tracking-[-0.4px] sm:text-[18px]">
                {title}
              </h3>
            ) : null}
            {description ? (
              <p className="text-base-600 mt-1.5 max-w-[540px] text-[12px] leading-[1.5] sm:text-[13px]">
                {description}
              </p>
            ) : null}
          </div>
...
export interface FormRowProps {
  ...
  label: string;
  /** Texte d'aide sous le champ (11px base-500). */
  helper?: string;
...
export function FormRow({ label, helper, required, span = 1, children }: FormRowProps) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${colSpan}`}>
      <label className="text-base-700 inline-flex items-center gap-1 text-[13px] font-medium">
        {label}
        {required ? <span className="text-danger-600">*</span> : null}
      </label>
      {children}
      {helper ? <p className="text-base-500 text-[11px] leading-[1.4]">{helper}</p> : null}
    </div>
  );
}
...
export function SaveSuccessBanner({
  message = (
    <>
      ...
      <span className="text-success-600">Visibles immédiatement.</span>
    </>
  ),
  whenAgo,
}: SaveSuccessBannerProps) {
  return (
    <div className="border-success-200 bg-success-50 mb-4 flex items-center gap-3 rounded-xl border px-4 py-3">
      <span className="bg-success-400 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-white">
        <Check className="h-3.5 w-3.5" strokeWidth={2.5} />
      </span>
      <div className="text-success-800 flex-1 text-[13px]">{message}</div>
      {whenAgo ? <span className="text-success-600 font-mono text-[11px]">{whenAgo}</span> : null}
    </div>
  );
}
