 ...
 * - tbody rows : border-bottom border-border, hover bg-background (subtil)
 * - tabular-nums sur les cellules monétaires (à appliquer côté usage)
 */
...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn('border-border bg-muted border-t font-medium [&>tr]:last:border-b-0', className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      'text-muted-foreground h-11 px-3 text-left align-middle text-xs font-medium tracking-widest uppercase [&:has([role=checkbox])]:pr-0',
      className,
    )}
    {...props}
  />
